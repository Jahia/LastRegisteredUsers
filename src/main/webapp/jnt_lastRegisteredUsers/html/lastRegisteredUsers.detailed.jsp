				 ...
				 <c:when test="${lastRegistered.nodes.size != 0}">
					 ...
					 <c:forEach items="${lastRegistered.nodes}" var="user">
					 	<c:if test="${user.name != 'guest'}" >
							...
							<strong><fmt:message key='jnt_lastRegisteredUsers.firstNameLabel'/>:</strong> ${user.propertiesAsString['j:firtsName']},
							<strong><fmt:message key='jnt_lastRegisteredUsers.lastNameLabel'/>:</strong> ${user.propertiesAsString['j:lastName']},
							<strong><fmt:message key='jnt_lastRegisteredUsers.emailLabel'/>:</strong> ${user.propertiesAsString['email']},
